...
import torch
from stout.distributed import DistributedInfo
...
class ServerArgs(SchedulerConfig):
    ...
    def zmq_frontend_addr(self) -> str:
        return "ipc:///tmp/stout_3" + self._unique_suffix

    @property
    def zmq_tokenizer_addr(self) -> str:
        if self.share_tokenizer:
            return self.zmq_detokenizer_addr
        result = "ipc:///tmp/stout_4" + self._unique_suffix
...
def parse_args(args: List[str], run_shell: bool = False) -> Tuple[ServerArgs, bool]:
    ...
    """
    from stout.attention import validate_attn_backend
    from stout.kvcache import SUPPORTED_CACHE_MANAGER
    from stout.moe import SUPPORTED_MOE_BACKENDS

    parser = argparse.ArgumentParser(description="Stout Server Arguments")
    ...
    if (dtype_str := kwargs["dtype"]) == "auto":
        from stout.utils import cached_load_hf_config
